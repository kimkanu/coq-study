...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | nil => true
  | v :: t =>
    match (beq_nat 0 (count v s2)) with
    | true => false
    | false => subset t (remove_one v s2)
    end
  end.
...
Proof.
  intros l1 l2. induction l1 as [|n1 t1 H].
  - simpl. rewrite -> app_nil_r. reflexivity.
  - simpl. rewrite -> H, app_assoc. reflexivity. Qed.
...
Proof.
  intros l. induction l as [|n t H].
  - reflexivity.
  - simpl. rewrite -> rev_app_distr, H. reflexivity. Qed.
...
Proof.
  intros l1 l2. induction l1 as [|n t H].
  - reflexivity.
  - destruct n as [|n'].
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | v :: t =>
    match (beq_nat v (hd 0 l2)) with
    | true => beq_natlist t (remove_one v l2)
    | false => false
    end
  end.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    assert (M: forall m: nat, true = beq_nat m m).
    { intros m. induction m as [|m' J].
      + reflexivity.
      + simpl. rewrite <- J. reflexivity. }
    rewrite <- M, H. reflexivity. Qed.
(** [] *)
...
Proof.
intros s. induction s as [|v t H].
  - reflexivity.
  - assert (M: forall m: nat, leb m (S m) = true).
    { intros m. induction m as [| m' J].
      * reflexivity.
      * simpl. rewrite -> J. reflexivity. }
    destruct v as [| v'].
...
Proof.
intros l default. destruct l as [| v t].
  - reflexivity.
  - reflexivity. Qed.
(** [] *)
...
assert (M: forall m: nat, true = beq_nat m m).
    { intros m. induction m as [|m' J].
      + reflexivity.
      + simpl. rewrite <- J. reflexivity. }
simpl. rewrite <- M. reflexivity. Qed.
(** [] *)
