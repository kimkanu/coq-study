...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros. symmetry. rewrite <- rev_involutive, H. reflexivity. Qed.
(** [] *)
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros. apply trans_eq with (m := minustwo o). rewrite -> H0. apply H. reflexivity. Qed.
...
Proof.
  ...
  - destruct m.
    ...
    + rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. simpl.
      intros T. inversion T. apply IHn' in H0. rewrite -> H0. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - destruct m.
    + intros H. inversion H.
    + simpl. intros H. apply IHn in H. rewrite -> H. reflexivity. Qed.
(** [] *)
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
     ...
     nth_error l n = None.
Proof.
  intros. generalize dependent n. generalize dependent l. induction l.
  - reflexivity.
  - intros. simpl in H. simpl.
    assert (T: Nat.pred n = length l).
    { rewrite <- H. reflexivity. }
    rewrite -> T. destruct n.
    + inversion H.
...
Proof.
  ...
  - intros. destruct x. simpl in H. destruct (split l).
    ...
    + destruct l2.
      * inversion H.
      * inversion H. simpl.
        assert (P: (l0, l3) = (l1, l2) -> combine l1 l2 = l).
        { apply IHl. }
        assert (Q: (l0, l3) = (l1, l2)).
        { rewrite -> H2, H4. reflexivity. }
        apply P in Q. rewrite -> Q. reflexivity. Qed.
(** [] *)

(** However, [destruct]ing compound expressions r
equires a bit of
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f. destruct (f true) eqn:H.
  - destruct (f false) eqn:J.
    + intros b. destruct b.
      * rewrite -> H, H, H. reflexivity.
      * rewrite -> J, H, H. reflexivity.
    + intros b. destruct b.
      * rewrite -> H, H, H. reflexivity.
      * rewrite -> J, J, J. reflexivity.
  - destruct (f false) eqn:J.
    + intros b. destruct b.
      * rewrite -> H, J, H. reflexivity.
      * rewrite -> J, H, J. reflexivity.
    + intros b. destruct b.
      * rewrite -> H, J, J. reflexivity.
      * rewrite -> J, J, J. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros. inversion H.
  - remember (test x0) as b. destruct b.
    + intros. symmetry in Heqb. simpl in H. rewrite -> Heqb in H. inversion H. rewrite <- H1. apply Heqb.
    + intros. symmetry in Heqb. simpl in H. rewrite -> Heqb in H. apply IHl in H. apply H. Qed.
(** [] *)
...
Theorem equiv_exists: forall (X: Type) (f: X -> bool) (l: list X),
  existsb f l = existsb' f l.
Proof.
  intros. induction l.
  - unfold existsb'. reflexivity.
  - unfold existsb'. simpl.
    assert (deMorgan: forall (b1 b2: bool), negb (b1 && b2) = negb b1 || negb b2).
    { destruct b1.
      - destruct b2.
        + reflexivity.
        + reflexivity.
      - destruct b2.
        + reflexivity.
        + reflexivity. }
    rewrite -> deMorgan. unfold existsb' in IHl. rewrite <- IHl.
