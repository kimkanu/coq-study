...
Lemma t_update_eq : forall A (m: total_map A) x v,
  (m & {x --> v}) x = v.
Proof.
  intros. unfold t_update. rewrite <- beq_string_refl. reflexivity. Qed.
(** [] *)
...
Theorem t_update_neq : forall (X:Type) v x1 x2
  ...
  (m & {x1 --> v}) x2 = m x2.
Proof.
  intros X v x1 x2 m. unfold t_update. rewrite <- (beq_string_false_iff x1 x2). intros. rewrite -> H. reflexivity. Qed.
(** [] *)
...
Proof.
  intros x y. apply iff_reflect. rewrite beq_string_true_iff. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - destruct (beq_stringP x2 x).
    + rewrite -> e, e0 in H. apply beq_string_false_iff in H. rewrite <- beq_string_refl in H. inversion H.
    + reflexivity.
  - destruct (beq_stringP x2 x).
    + reflexivity.
    + reflexivity. Qed.
