...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun t => andb (negb (leb t 7)) (evenb t)) l.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (filter test l, filter (fun v => negb(test v)) l).
...
Proof.
  ...
  - reflexivity.
  - simpl.
    assert (M: forall l': list X, map f (l' ++ [x]) = map f l' ++ [f x]).
    { induction l'.
      + reflexivity.
      + simpl. rewrite -> IHl'. reflexivity. }
    rewrite -> M, IHl. reflexivity. Qed.
(** [] *)
...
*)
Theorem nth_error_none : forall X n l, length l = n -> @nth_error X l n = None.
Proof.
  intros X n. induction n. 
  - intros l. destruct l.
    + reflexivity.
    + simpl. intros H. inversion H.
  - intros. destruct l as [| v t].
    + inversion H.
    + simpl.
      assert (P: forall m: nat, pred (S m) = m).
      { intros. reflexivity. }
      assert (L: n = length t).
      { simpl in H. rewrite <- P. rewrite -> H, P. reflexivity. }
      rewrite -> IHn.
      reflexivity. rewrite -> L. reflexivity. Qed.
(** [] *)
...
Definition mult (n m : nat) : nat :=
  fun (X: Type) (f: X -> X) (x: X) => n X (fun y => m X f y) x.
...
(* 
fun (X: Type) (f: X -> X) (x: X) => fold (fun a b => mult a b) (m (list nat) (fun (t: list nat) => (n :: t)) []) one.
makes a universe inconsistency error.
 *)
