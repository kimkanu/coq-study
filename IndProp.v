...
Proof.
  induction n.
  - simpl. apply ev_0.
  - simpl. apply (ev_SS (double n) IHn). Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl. apply ev_SS. apply IHE in H. apply H. Qed.
(** [] *)
...
Proof.
  split.
  - intros. induction H. apply ev_0. apply (ev_SS 0 ev_0). apply (ev_sum n m). apply IHev'1. apply IHev'2.
  - intros. induction H. apply ev'_0. apply (ev'_sum 2 n). apply ev'_2. apply IHev. Qed.
(** [] *)
...
Proof.
  intros. induction H0.
  - simpl in H. apply H.
  - simpl in H. apply evSS_ev in H. apply IHev in H. apply H. Qed.
(** [] *)
...
Proof.
  intros.
  assert (ev((n+m)+(n+p))). { apply ev'_ev. apply (ev'_sum (n+m) (n+p)). apply ev'_ev. apply H. apply ev'_ev. apply H0. }
  assert (n+m = m+n). { rewrite -> plus_comm. reflexivity. }
  rewrite -> H2 in H1.
  rewrite <- plus_assoc with (m) (n) (n+p) in H1.
  rewrite -> plus_assoc with (n) (n) (p) in H1.
  assert (n+n = double n). { rewrite <- double_plus. reflexivity. }
  rewrite -> H3 in H1. rewrite -> plus_assoc in H1. rewrite -> plus_comm with (m) (double n) in H1.
  assert (ev (double n)). { apply ev_double. } apply (ev_ev__ev (double n) (m+p)).
  rewrite <- plus_assoc in H1. apply H1. apply H4. Qed.
(** [] *)
...
Inductive total_relation : nat -> nat -> Prop :=
  | total_relation_n_m : forall n m: nat, total_relation n m.
(** [] *)
...
Proof.
  ...
  - intros n. destruct n.
    + intros. apply O_le_n.
    + intros. inversion H. apply le_n. apply IHm in H1. apply le_S. apply H1. Qed.

Theorem le_plus_l : forall a b,
  a <= a + b.
Proof.
  intros. generalize dependent a. induction b.
  - intros. rewrite <- plus_n_O. apply le_n.
  - intros. rewrite <- plus_n_Sm. apply le_S. apply IHb. Qed.
...
Proof.
  unfold lt.
  intros. destruct n1.
  - simpl in H. destruct m. inversion H. split. apply n_le_m__Sn_le_Sm. apply O_le_n. apply H.
  - simpl in H. destruct m. inversion H. destruct m. apply Sn_le_Sm__n_le_m in H. inversion H.
    apply Sn_le_Sm__n_le_m in H. apply Sn_le_Sm__n_le_m in H. split.
    apply n_le_m__Sn_le_Sm. apply n_le_m__Sn_le_Sm. induction n2.
    + rewrite <- plus_n_O in H. apply H.
    + rewrite <- plus_n_Sm in H. destruct m. inversion H. apply Sn_le_Sm__n_le_m in H.
      assert (m <= S m). { apply le_S. apply le_n. }
      assert (n1 + n2 <= S m). { apply (le_trans (n1 + n2) (m) (S m)). apply H. apply H0. }
      apply IHn2 in H1. apply H1. 
    + apply n_le_m__Sn_le_Sm. induction n1.
      * simpl in H. assert (m <= S m). { apply le_S. apply le_n. }
        apply (le_trans (n2) (m) (S m)). apply H. apply H0.
      * assert (n1 + n2 <= S n1 + n2). { simpl. apply le_S. apply le_n. }
        assert (n1 + n2 <= m). { apply (le_trans (n1 + n2) (S (n1 + n2)) (m)).
        apply H0. apply H. } apply IHn1 in H1. apply H1. Qed.
        
Theorem lt_S : forall n m,
  n < m ->
  n < S m.
Proof.
  unfold lt. intros. apply n_le_m__Sn_le_Sm. destruct m. inversion H. apply Sn_le_Sm__n_le_m in H. apply le_S. apply H. Qed.
...
Proof.
  ...
  - generalize dependent n. induction m.
    + destruct n. intros. apply le_S. apply le_n. destruct n. intros. apply le_n.
      intros. inversion H.
    + destruct n. intros. apply O_le_n.
      intros. simpl in H. apply IHm in H. apply n_le_m__Sn_le_Sm. apply H. Qed.
...
Proof.
  ...
  - generalize dependent n. induction m.
    + intros. inversion H. reflexivity. inversion H1. reflexivity.
    + destruct n. simpl. reflexivity.
      simpl. intros. apply IHm. intros. assert (S n <= S m). { apply n_le_m__Sn_le_Sm. apply H0. }
      apply IHm0 in H1. apply H1. apply Sn_le_Sm__n_le_m in H. apply H. Qed.
...
Proof.
  intros. apply (leb_complete n m) in H. apply (leb_complete m o) in H0. apply (leb_correct n o). apply (le_trans n m o). apply H. apply H0. Qed.
(** [] *)
...
Theorem leb_iff : forall n m,
  leb n m = true <-> n <= m.
Proof.
  split. apply (leb_complete n m). apply (leb_correct n m). Qed.
...
Proof. 
  ...
  - generalize dependent o. generalize dependent n. induction m.
    -- induction n.
      ...
      + intros. simpl in H. simpl in IHn. destruct o. inversion H. apply S_injective in H. apply IHn in H. apply c3. apply H.
    -- induction n.
      + intros. simpl in H. destruct o. inversion H. apply S_injective in H. apply IHm in H. apply c2. apply H.
      + intros. simpl in H. rewrite <- plus_n_Sm in H. destruct o. inversion H. apply c2.
        assert (R m (S n) (S (m + n))). { apply IHm. rewrite <- plus_n_Sm. reflexivity. }
        apply S_injective in H. rewrite <- H. apply H0.
  - intros R. induction R.
    + reflexivity.
    + simpl. rewrite -> IHR. reflexivity.
    + rewrite <- plus_n_Sm. unfold fR in IHR. rewrite -> IHR. reflexivity.
    + unfold fR in IHR. rewrite <- plus_n_Sm in IHR. apply S_injective in IHR. simpl in IHR. apply S_injective in IHR. apply IHR.
    + rewrite -> plus_comm in IHR. apply IHR. Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | s2: forall (n: nat) (l1 l2: list nat), subseq l1 l2 -> subseq (n :: l1) (n :: l2)
  | s3: forall (n: nat) (l1 l2: list nat), subseq l1 l2 -> subseq l1 (n :: l2).

Theorem subseq_refl : forall l: list nat, subseq l l.
Proof. induction l.
  - apply s1.
  - apply s2. apply IHl. Qed.

Theorem subseq_app : forall l1 l2 l3: list nat, subseq l1 l2 -> subseq l1 (l2 ++ l3).
Proof.
  assert (H0: forall (n:nat) (l:list nat), [n]++l = n::l). { reflexivity. }
  intros l1 l2 l3 H. induction H.
  - apply s1.
  - rewrite <- H0 with (n) (l2). rewrite <- app_assoc. apply s2. apply IHsubseq.
  - rewrite <- H0 with (n) (l2). rewrite <- app_assoc. apply s3. apply IHsubseq.  Qed.
...
Lemma MUnion' : forall T (s : list T) (re1 re2 : @reg_exp T),
  ...
  s =~ Union re1 re2.
Proof.
  intros. destruct H. apply MUnionL. apply H. apply MUnionR. apply H. Qed.
...
Proof.
  ...
  - simpl. simpl in H. apply MStarApp.
    + destruct (H x). left. reflexivity. apply MEmpty. apply MChar. apply MApp. apply e1. apply e2. apply MUnionL. apply e. apply MUnionR. apply e.
      apply MStar0. apply MStarApp. apply e1. apply e2.
    + apply IHss. intros. assert (x = s \/ In s ss). { right. apply H0. }
      assert (s =~ re). { apply H. apply H1. } apply H2. Qed.
(** [] *)
...
Proof.
  symmetry. split.
  - (* easy direction <- *) generalize dependent s5. induction s4.
    + intros. rewrite <- H. simpl. apply MEmpty.
    + intros. rewrite <- H. simpl. assert (x::s4 = [x]++s4). { reflexivity. }
      rewrite -> H0. apply MApp. apply MChar. assert (s4 =~ reg_exp_of_list s4).
      { apply IHs4. reflexivity. } apply H1.
  - (* -> *) assert (H6: forall (t: T) (s: list T), t::s = [t]++s). { reflexivity. }
      generalize dependent s4. induction s5.
    + simpl. intros. inversion H. reflexivity.
    + simpl. intros. destruct s4. inversion H.
      assert (s4 = []). { destruct s4. reflexivity. rewrite -> H6 in H0.
      rewrite <- app_assoc in H0. inversion H0. }
      rewrite -> H5 in H3. inversion H3.
      rewrite -> H6. inversion H. inversion H3. rewrite -> H8 in H5. rewrite <- H5 in H0.
      simpl in H0. simpl. rewrite <- H0. destruct H0. apply IHs5 in H4. rewrite -> H4. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - induction re.
    ...
    + simpl. exists [t]. apply MChar.
    + simpl. intros. apply andb_true_iff in H. destruct H. apply IHre1 in H. destruct H.
    ...
    + simpl. intros. exists []. apply MStar0. Qed. 
...
Proof.
  ...
  - inversion Heqre'. apply IHexp_match2 in Heqre'. destruct Heqre'. destruct H1. exists ([s4] ++ x).
    simpl. rewrite <- H1. split. reflexivity.
    intros s' H'. inversion H'. rewrite <- H4. rewrite <- H2. apply H. apply H3. apply H4. Qed.
(** [] *)
...
Proof.
  ...
  - (* MApp *)
    ...
    { omega. }
    destruct H0. apply IH1 in H0. destruct H0. destruct H0. destruct H0.
    destruct H0. destruct H1. exists x, x0, (x1 ++ s2). rewrite -> H0. split.
    ...
    rewrite -> H3. apply (MApp (x ++ napp m x0 ++ x1) _ s2). apply H2. apply Hmatch2.

    apply IH2 in H0. destruct H0. destruct H0. destruct H0.
    destruct H0. destruct H1. exists (s1 ++ x), x0, x1. rewrite -> H0. split.
    rewrite <- app_assoc. reflexivity.
    split. apply H1. intros.
    assert ((s1 ++ x) ++ napp m x0 ++ x1 = s1 ++ (x ++ napp m x0 ++ x1)). 
    { rewrite -> app_assoc. rewrite -> app_assoc. rewrite -> app_assoc. reflexivity. }
    rewrite -> H3. apply (MApp s1 _ (x ++ napp m x0 ++ x1)). apply Hmatch1. apply H2.
  - (* MUnionL *)
    simpl. intros. assert (pumping_constant re1 <= length s1). { omega. }
    apply IH in H0. destruct H0. destruct H0. destruct H0. destruct H0. destruct H1.
    exists x, x0, x1. split. apply H0. split. apply H1. intros.
    apply (MUnionL (x ++ napp m x0 ++ x1) _ _). apply H2.
  - (* MUnionR *)
    simpl. intros. assert (pumping_constant re2 <= length s2). { omega. }
    apply IH in H0. destruct H0. destruct H0. destruct H0. destruct H0. destruct H1.
    exists x, x0, x1. split. apply H0. split. apply H1. intros.
    apply (MUnionR _ (x ++ napp m x0 ++ x1) _). apply H2.
  - (* MStar0 *)
    simpl. omega.
  - (* MStarApp *)
    simpl. remember (length s2) as l2. destruct l2.
    + rewrite -> app_length. rewrite <- Heql2. rewrite <- plus_n_O.
      assert (s2 = []). { induction s2. reflexivity. inversion Heql2. }
      rewrite -> H. rewrite -> app_nil_r. exists [], s1, [].
      split. simpl. symmetry. apply app_nil_r. split. unfold not. intros.
      assert (length s1 = 0). { rewrite -> H1. reflexivity. }
      rewrite -> H2 in H0. omega.
      intros. simpl. rewrite -> app_nil_r. induction m.
      * simpl. apply MStar0.
      * simpl. apply MStarApp. apply Hmatch1. apply IHm.
    + simpl in IH2. assert (1 <= S l2). { omega. } apply IH2 in H.
      destruct H. destruct H. destruct H. destruct H. destruct H0.
      intros. exists (s1 ++ x), x0, x1. rewrite -> H. split.
      rewrite -> app_assoc. rewrite -> app_assoc. reflexivity.
      split. apply H0. intros. assert ((s1 ++ x) ++ napp m x0 ++ x1 = s1 ++ (x ++ napp m x0 ++ x1)).
      { rewrite -> app_assoc. rewrite -> app_assoc. rewrite -> app_assoc. reflexivity. }
      rewrite -> H3. apply MStarApp. apply Hmatch1. apply H1. Qed.
...
Proof.
  intros. split.
  - intros. destruct b. reflexivity. inversion H.
    assert (P /\ ~P). { split. apply H0. apply H1. }
    apply (contradiction_implies_anything P (false = true)). apply H2.
  - intros. rewrite -> H0 in H. inversion H. apply H1. Qed.
(** [] *)
...
Proof.
  ...
  - (* l = m :: l' *)
    ...
    + (* n = m *)
      intros. inversion H0.
    + (* n <> m *)
      simpl. intros H'. apply IHl' in H'. unfold not. intros. destruct H0.
      apply (contradiction_implies_anything (n = m) False). split. symmetry. apply H0. apply H.
      apply (contradiction_implies_anything (In n l') False). split. apply H0. apply H'. Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | NEmpty : nostutter []
  | NChar : forall (x: X), nostutter [x]
  | NNotCons : forall (x y: X) (l: list X), (nostutter (y :: l) /\ (x <> y) -> nostutter (x :: y :: l)).
...
| IOEmptyL: forall l, inorder_merge [] l l
| IOEmptyR: forall l, inorder_merge l [] l
| IOAppL: forall (l1 l2 l: list X) (x: X), (inorder_merge l1 l2 l -> inorder_merge (x :: l1) l2 (x :: l))
| IOAppR: forall (l1 l2 l: list X) (x: X), (inorder_merge l1 l2 l -> inorder_merge l1 (x :: l2) (x :: l)).

Theorem inorder_merge_filter : forall (X: Type) (test: X -> bool) (l l1 l2: list X),
  inorder_merge l1 l2 l /\ (forall x: X, In x l1 -> test x = true) /\ (forall x: X, In x l2-> test x = false)
  -> filter test l = l1.
Proof.
  intros X test.
  assert (F: forall s: list X, (forall y : X, In y s -> test y = false) -> filter test s = []).
    { induction s. 
      - intros. reflexivity.
      - intros. simpl in H. simpl. assert (test x = false). { apply H. left. reflexivity. }
        rewrite -> H0. apply IHs. intros. apply H. right. apply H1. }
  assert (T: forall s: list X, (forall y : X, In y s -> test y = true) -> filter test s = s).
    { induction s. 
      - intros. reflexivity.
      - intros. simpl in H. simpl. assert (test x = true). { apply H. left. reflexivity. }
        rewrite -> H0. assert (forall y : X, In y s -> test y = true). { intros. apply H. right. apply H1. }
        apply IHs in H1. rewrite -> H1. reflexivity. }
  induction l.
  - intros. destruct H. inversion H. reflexivity. reflexivity.
  - intros. destruct H. destruct H0. inversion H. rewrite <- H4. apply F. apply H1.
    rewrite <- H4. apply T. apply H0. simpl. assert (test x = true). { rewrite <- H2. apply H0. rewrite <- H3. simpl. left. reflexivity. }
    rewrite -> H7. assert (IHl': filter test l = l0). { apply (IHl l0 l2). split. apply H5. split. intros. apply H0. rewrite <- H3.
    simpl. right. apply H8. apply H1. }
    rewrite -> IHl'. reflexivity.

    simpl. assert (test x = false). { rewrite <- H2. apply H1. rewrite <- H4. simpl. left. reflexivity. }
    rewrite -> H7. apply (IHl l1 l3). split. apply H5. split. apply H0. intros. apply H1. rewrite <- H4. simpl. right. apply H8. Qed.
(** [] *)
...
Theorem filter_longest : forall (X: Type) (test: X -> bool) (l l1: list X),
  (filter test l1 = l1 /\ exists l2, inorder_merge l1 l2 l) -> length l1 <= length (filter test l).
Proof.
  intros X test. 
  assert (FT: forall s, filter test (filter test s) = filter test s).
      { induction s. 
        - reflexivity.
        - remember (test x) as b. destruct b.
          + simpl. rewrite <- Heqb. simpl. rewrite <- Heqb. rewrite -> IHs. reflexivity.
          + simpl. rewrite <- Heqb. rewrite -> IHs. reflexivity. }
  induction l.
  - intros. destruct H. inversion H0. inversion H1. reflexivity. reflexivity.
  - intros. destruct H. inversion H0. inversion H1.
    + simpl. apply le_0_n.
    + rewrite <- H4. rewrite <- H. 
      rewrite -> FT. reflexivity.
    + rewrite <- H2. rewrite -> H3. remember (test x1) as b. destruct b.
      * simpl. rewrite <- Heqb. simpl. rewrite <- H3. simpl. apply n_le_m__Sn_le_Sm. apply IHl. split.
        rewrite <- H3 in H. simpl in H. rewrite <- Heqb in H. inversion H. apply FT. exists x0. apply H5.
      * simpl. rewrite <- H3 in H. inversion H. rewrite <- Heqb in H8.
        assert (forall l0, length (filter test l0) <= length l0).
        { induction l4. reflexivity. simpl. remember (test x2) as b. destruct b.
          - simpl. apply n_le_m__Sn_le_Sm. apply IHl4.
          - simpl. apply le_S. apply IHl4. }
        assert (length (filter test l0) <= length l0). { apply H7. }
        rewrite <- Heqb. rewrite -> H8 in H9. simpl in H9.
        assert (forall n, ~ (S n <= n)). { induction n. unfold not. intros. inversion H10. unfold not. intros. apply Sn_le_Sm__n_le_m in H10.
        apply (contradiction_implies_anything (S n <= n)). split. apply H10. apply IHn. }
        apply (contradiction_implies_anything (S (length l0) <= length l0)). split. apply H9. apply H10.
    + simpl. rewrite <- H2. remember (test x1) as b. destruct b.
      * simpl. apply le_S. apply IHl. split. apply H. exists l2. apply H5.
      * apply IHl. split. apply H. exists l2. apply H5. Qed.
...
Proof.
  ...
  - intros. inversion H. destruct s4.
    ...
    + right. simpl in H0. inversion H0. exists s4, s5. split.
      * reflexivity.
      * split. rewrite <- H6. apply H3. apply H4.
  - intros. destruct H.
    + destruct H. apply (MApp []). apply H. apply H0.
    + destruct H. destruct H. destruct H. rewrite -> H.
      assert (a :: x ++ x0 = (a :: x) ++ x0). { reflexivity. }
      rewrite -> H1. destruct H0. apply MApp. apply H0. apply H2. Qed.
...
Proof.
  intros. split.
  - intros. inversion H. destruct s4.
    + destruct s5. inversion H0. simpl in H0. inversion H0. rewrite -> H5 in H3. rewrite -> H6 in H3.
      inversion H3. destruct s4. simpl in H4.
(** [] *)
