...
Proof.
  intros. 
  assert (forall a b: nat, a + b = 0 -> a = 0).
  { destruct b.
    + intros. rewrite <- plus_n_O in H0. apply H0.
    + intros. rewrite <- plus_n_Sm in H0. inversion H0. }
  apply H0 in H as H1.
  split.
  - apply H1.
  - rewrite -> plus_comm in H. apply H0 in H. apply H. Qed.
...
Proof.
  intros. unfold not in H0. unfold not. intros. apply H in H1. apply H0 in H1. apply H1. Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  unfold not. intros. destruct H. apply H0 in H. apply H. Qed.
(** [] *)
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros. destruct H. destruct H0. split. intros. apply H in H3. apply H0 in H3. apply H3.
  intros. apply H2 in H3. apply H1 in H3. apply H3. Qed.
(** [] *)
...
Proof.
  unfold not. intros. destruct H0 as [y E]. assert (T: P y). { apply H. } apply E in T. apply T. Qed.
(** [] *)
...
Proof.
  ...
  - split.
    + intros. inversion H.
    + intros. inversion H. destruct H0. inversion H1.
  - split.
    + intros. simpl in H. destruct H.
      * exists x. split. apply H. simpl. left. reflexivity.
      * apply IHl in H. destruct H. exists x0. split. apply H. simpl. right. apply H.
    + intros. simpl. destruct H. destruct H. simpl in H0. destruct H0.
      * rewrite <- H0 in H. left. apply H.
      * right. apply IHl. exists x0. split. apply H. apply H0. Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl. split. intros. right. apply H. intros. destruct H. inversion H. apply H.
  - intros. simpl. split. intros. rewrite <- or_assoc. destruct H.
    + left. apply H.
    + apply IHl in H. right. apply H.
    + rewrite -> IHl. rewrite <- or_assoc. intros. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - split. intros. simpl in H.
    + simpl. apply I.
    + intros H. simpl. intros. exfalso. apply H0.
  - split. intros. simpl in H. simpl. rewrite <- IHl. split. apply H. left. reflexivity.
    intros.
    assert (W: forall P Q R: Prop, (P \/ Q -> R) -> (Q -> R)).
    { intros. assert (W0: P0 \/ Q). right. apply H2. apply H1 in W0. apply W0. }
    assert (V: (x = x0 \/ In x0 l -> P x0) -> In x0 l -> P x0). 
    { apply W with (P:= (x=x0)) (Q:=In x0 l). }
    assert (U: x = x0 \/ In x0 l -> P x0). 
    { apply H. }
    apply V in U. apply U. apply H0. intros. simpl in H. simpl in H0. destruct H0.
    + rewrite -> H0 in H. apply H.
    + rewrite <- IHl in H. apply H in H0. apply H0. Qed.
(** [] *)
...
Proof.
  ...
  - unfold oddb in H2. rewrite <- Heqevenb in H2. simpl in H2.
    assert (T: false = false). { reflexivity. } apply H2 in T. apply T.
  - unfold oddb in H1. rewrite <- Heqevenb in H1. simpl in H1.
    assert (T: true = true). { reflexivity. } apply H1 in T. apply T. Qed.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros. remember (oddb n) as oddb. 
  assert (forall x y, x = y -> negb x = negb y).
  { intros. rewrite -> H1. reflexivity. }
  assert (forall x y, negb x = negb y -> x = y).
  { intros. rewrite <- negb_involutive. symmetry. rewrite <- negb_involutive.
    rewrite -> H2. reflexivity. }
  destruct oddb.
  - unfold combine_odd_even in H. unfold oddb in Heqoddb. symmetry in Heqoddb.
    rewrite <- negb_involutive in Heqoddb.
    apply H2 in Heqoddb. rewrite -> Heqoddb in H. simpl in H. apply H.
  - inversion H0. Qed.
...
Proof.
  ...
  - unfold combine_odd_even in H. unfold oddb in Heqoddb. symmetry in Heqoddb.
    rewrite <- negb_involutive in Heqoddb.
    apply H2 in Heqoddb. rewrite -> Heqoddb in H. simpl in H. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - rewrite -> evenb_S. remember (evenb n) as e. destruct e.
    + simpl. destruct IHn. exists x. rewrite <- H. reflexivity.
    + simpl. destruct IHn. exists (S x). simpl. rewrite <- H. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. split.
  - intros. destruct b1. destruct b2. split. reflexivity. reflexivity. inversion H. inversion H.
  - intros. destruct H. rewrite -> H, H0. reflexivity. Qed.

Lemma orb_true_iff : forall b1 b2,
  b1 || b2 = true <-> b1 = true \/ b2 = true.
Proof.
  intros. split.
  - intros. destruct b1. destruct b2. left. reflexivity. left. reflexivity. destruct b2. right. reflexivity. inversion H.
  - intros. destruct H. rewrite -> H. simpl. reflexivity. rewrite -> H. destruct b1. reflexivity. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. split.
  - unfold not. intros. rewrite -> H0 in H. rewrite <- beq_nat_refl in H. inversion H.
  - unfold not. intros. remember (beq_nat x y) as b. destruct b.
    + symmetry in Heqb. apply beq_nat_true in Heqb. apply H in Heqb. inversion Heqb.
    + reflexivity. Qed.
(** [] *)
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  | x1 :: t1 =>
    match l2 with
    | [] => false
    | x2 :: t2 => (beq x1 x2) && (beq_list beq t1 t2)
    end
  end.
...
- destruct l2.
  ...
  + split. intros. simpl in H0.
    assert (forall b1 b2: bool, b1 && b2 = true -> b2 = true).
    { destruct b2. destruct b1. reflexivity. reflexivity.
      destruct b1. intros. inversion H1. intros. inversion H1. }
    inversion H0. apply H1 in H3.
    rewrite -> andb_commutative in H0. apply H1 in H0.
    apply H in H0. apply IHl1 in H3. rewrite -> H0. rewrite <- H3. reflexivity.
    intros. inversion H0.
    assert (l1 = l1). { reflexivity. }
    simpl. apply IHl1 in H1.
    rewrite <- H3. rewrite -> H1.
    assert (a = a). { reflexivity. }
    apply H in H4. rewrite -> H4. reflexivity. Qed.
(** [] *)
...
Proof.
  assert (forall b1 b2: bool, b1 && b2 = true -> (b1 = true) /\ (b2 = true)).
    { destruct b2. destruct b1. split. reflexivity. reflexivity. intros. inversion H.
      destruct b1. intros. inversion H. intros. inversion H. }
  intros. split.
  induction l.
  - intros. simpl. apply I.
  - simpl. intros. apply H in H0. destruct H0. apply IHl in H1. split. apply H0. apply H1.
  - intros. induction l. reflexivity. simpl. simpl in H0. destruct H0. apply IHl in H1. rewrite -> H0, H1. reflexivity. Qed.
...
Proof.
  ...
  assert (P' x \/ ~ P' x). { apply H with (P := P' x). }
  destruct H1.
  - apply H1.
  - assert (exists x : X, ~ P' x). { exists x. apply H1. }
    unfold not in H0. apply H0 in H2. inversion H2. Qed.
(** [] *)
